...
5. For RAG, document is split and stored with embeddings in vector database.
...
2. **Vector Database**: Vector databases store and search high-dimensional vector embeddings, which are numerical representations of unstructured data like text, images, and audio, using specialized similarity search algorithms to find data based on contextual meaning rather than exact matches. A vector database is a full-fledged, comprehensive system that builds upon the functionality of a vector store, offering robust features such as distributed storage, scaling, security, data management, and support for complex queries to handle large-scale vector data.
3. **Embeddings**: Embeddings are the semantic backbone of LLMs, the gate at which raw text is transformed into vectors of numbers that are understandable by the model.
4. **Langchain**: LangChain is a software framework that helps facilitate the integration of large language models into applications.
5. **Token**: Tokens are words, character sets, or combinations of words and punctuation that are generated by large language models (LLMs) when they decompose text.
6. **Vector Store**: A vector store is a specialized component that optimizes for storing and retrieving high-dimensional vector embeddings, often as a lightweight library for specific tasks like similarity search.
